...
<div style="text-align:center;color:red;">
	...
	if($_REQUEST['result'] == "1"){
		echo "The payment added successfully.";
	}else if($_REQUEST['result'] == "2"){
		echo "The payment added successfully. But the email do not send successfully. Please contact <a href='mailto:windy.wang@expacta.com.cn'>windy.wang@expacta.com.cn</a>";
	}
	?>
</div>
...
<table cellspacing="1" class="width100" align="center">
	...
	<tbody>
	<tr>
		<td class="form-title">Payment Application</td>
	</tr>
	<tr>
		<td style="padding-left: 0px;display:none" id="show_list">
			<table cellspacing="1" class="width50" align="" id="employee_list" style="border-bottom:0px;">
				<tr class="row-category">
					...
					<td style="width:250px;">Payment Date</td>
					<td style="width:150px;">Explanation</td>
			...
			<table cellspacing="1" class="width50" style="border-top:0px;">
				<tr>
					<td>Payment Subject: <input type="text" id="payment_subject" name="payment_subject" style="width:400px;"/></td>
					<td style="text-align:right"><input type="submit" value="Save Payment" name="save_payment"/></td>
				</tr>
			</table>
...
<table cellspacing="1" class="width100" align="center">
	...
	<tr class="row-1">
		<td class="category">Explanation</td>
		<td>
			<input type="text" value="" size="75" name="explanation" id="explanation">
			<span id="explanation_error" style="color:red;"></span>
		</td>
	</tr>
 ...
 <!-- Without LDAP Email -->
	...
	<tr class="row-2">
		...
		<td>￥
			<input type="text" value="" size="5" name="amount" id="amount">
	...
	<tr>
		...
		<!-- Update Button -->
		<td>
			<input type="button" value="Add Payment" class="button" onclick="addPayment();">
		</td>
	</tr>
	</tbody>
</table>
...
function addPayment(){
	...
	if ($('#explanation').val() == "") {
		checkAdd = false;
		$('#explanation_error').html("Please input explanation");
	}
	if (!$('#amount').val().match(/^[0-9]{1,}$/)) {
		checkAdd = false;
	...
	if(checkAdd == true){
		...
		var paymentDateHours = $('#start_year').val() + "-" + $('#start_month').val() + '-' + $('#start_day').val();
		var paymentdate = $('#start_year').val() + "-" + $('#start_month').val() + '-' + $('#start_day').val();
		var paymentExplanation = $('#explanation').val(); 
		var paymentAmount = $('#amount').val(); 
		var trStr = '<tr class="row-1"><td align="left" style="text-align: center;" class="small-caption">'+userName+'<input name="employee_ids[]" value="'+userId+'" type="hidden"></td><td style="text-align: center;" class="small-caption">'+paymentDateHours+'<input type="hidden" name="payment_dates[]" value="'+paymentdate+'"></td><td align="center" style="text-align: center;" class="small-caption">'+paymentExplanation+'<input type="hidden" name="payment_explanation[]" value="'+paymentExplanation+'"></td><td align="center" style="text-align: center;" class="small-caption">￥'+paymentAmount+'<input type="hidden" name="payment_amount[]" value="'+paymentAmount+'"></td><td class="small-caption"><input type="button" value="Delete" onclick="removePayment(this)"></td></tr>';
